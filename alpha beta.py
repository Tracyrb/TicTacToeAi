...
def alpha_beta(alpha, beta, minimax_board, depth, is_maximizing):
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    score = alpha_beta(alpha, beta, minimax_board, depth + 1, False)
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    score = alpha_beta(alpha, beta, minimax_board, depth + 1, True)
                    minimax_board[row][col] = 0
...
def best_move():
    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if board[row][col] == 0:
                board[row][col] = 2
                score = alpha_beta(-1000, 1000, board, 0, False)
                board[row][col] = 0
                if score > best_score:
                    best_score = score
                    move = (row, col)
